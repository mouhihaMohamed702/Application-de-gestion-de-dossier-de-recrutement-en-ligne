    ...
    public function index()
    {
        if (Auth::user()->type == "candidat") {
            ...
            if (count($cvs) == 0 || count($candidates) == 0) {
                ...
                $var1=explode("/storage",Storage::url($candidate->picture));
                $var2=$var1[0]."/".$candidate->picture;
                 $skills = DB::table('skills')->where('user_id', Auth::user()->id)->Where('deleted_at', null)->get();
             //   return view('candidate.candidate-profile', [
               //     'cv' => $cv,
               //     'candidate' => $candidate,
              //      'img' => "http://127.0.0.1:8000/storage/picture/ez1hHEegJTijz213Xyuohm2C44d1U5XAtWXt5aBT.jpeg",
              //  ]);
              return View('candidate.candidate-profile')->with('cv',$cv)->with('candidate',$candidate)->with('img',$var2);
